...
export class SMSServer extends ServerSystem {
  ...
  register() {
    this._aServer.post("/api/sms/incoming", (req, res, next) => this._executeCommand(
      new SMSWebhookRequestOf(req, TWILIO_AUTH_TOKEN!, TWILIO_WEBHOOK_URL!),
      res,
      next
    ));
    this._aServer.post("/api/sms/incoming-test",  new OnlyRoles(this._users, 'Admin').allowedRole(),
     (req, res, next) => this._executeCommandTest(new AuthorizedRequestOf(req), res, next));
  }

  private async _executeCommand(req: SMSWebhookRequestOf, res: Response, next: NextFunction): Promise<void> {
    await this.executeAction(async () => {
      const command: Command = await new Commands(this._beneficiaries).of(
        new ParsedIncomingSms(new IncomingSms(req.body()))
      );
      new SMS (
  ...
  private async _executeCommandTest(req: AuthorizedRequestOf, res: Response, next: NextFunction): Promise<void> {
    await this.executeAction(async () => {
      await (await this._users.findOneBy(req.username())).ifActiveDo(async () => {
        const command: Command = await new Commands(new Beneficiaries()).of(
          new ParsedIncomingSms(new IncomingSms(req.body()))
        );
