...
import { CompleteOrderData, completeOrderSerializer } from "../beneficiaries/serializers/complete-order.serializer";
// TODO
...
export class OrdersServer extends ServerSystem {
  ...
  register() {
    this._aServer.post('/api/orders', new OnlyRoles(this._users, ['Admin', 'Merchant']).allowedRole(),
      (req, res, next) => this._createOrder(new AuthorizedRequestOf(req), res, next));
    this._aServer.post('/api/orders/complete', new OnlyRoles(this._users, ['Admin', 'Merchant']).allowedRole(),
      (req, res, next) => this._completeOrder(new AuthorizedRequestOf(req), res, next));
    this._aServer.get('/api/orders/beneficiaries/:phoneNumber', new OnlyRoles(this._users, ['Admin', 'NGO']).allowedRole(),
      (req, res, next) => this._findLatestOrdersByBeneficiary(new AuthorizedRequestOf(req), res, next));
    this._aServer.get('/api/orders/:orderId/status', new OnlyRoles(this._users, ['Admin', 'Merchant']).allowedRole(),
      (req, res, next) => this._getOrderStatus(new AuthorizedRequestOf(req), res, next));
  ...
  private async _findLatestOrdersByBeneficiary(req: AuthorizedRequestOf, res: Response, next: NextFunction): Promise<void> {
    await this.executeAction(async () => {
      // TODO:
      // return await (await this._users.findOneBy(req.username())).ifActiveDo<void>(async () => {
  ...
  private async _getOrderStatus(req: AuthorizedRequestOf, res: Response, next: NextFunction): Promise<void> {
    await this.executeAction(async () => {
      return await (await this._users.findOneBy(req.username())).ifActiveDo<void>(async () => {
        const order = await this._orders.findOneById(
          req.params().orderId
        );
